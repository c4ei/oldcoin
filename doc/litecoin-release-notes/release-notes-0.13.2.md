MiningTT Core version 0.13.2 is now available from:

  <https://download.miningTT.org/miningTT-0.13.2.1/>
...
updates will be released anymore. Without security updates, using a miningTT
...
ECDSA signatures inside MiningTT transactions now use validation using
...
Previous versions of MiningTT Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of MiningTT Core was to raise the default minimum
relay fee.

MiningTT Core 0.13.2 will have a strict maximum size on the mempool. The
...
MiningTT Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running miningTTd without having to do any manual configuration.
...
MiningTT Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. MiningTT Core will relay transactions with
...
entirely in MiningTT Core 0.13, and it is at this time undecided whether the
...
MiningTT Core has been updated to make use of this.

This means that if Tor is running (and proper authorization is available),
MiningTT Core automatically creates a hidden service to listen on, without
manual configuration. MiningTT Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if MiningTT Core is listening, and
...
MiningTTd can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals MiningTT
Core to use floating fees. By default, MiningTT Core will use floating
fees.
...
(default: `0.0002` MGTT/kB).

At all times, MiningTT Core will cap fees at `-maxtxfee=<x>` (default:
0.10) MGTT.
Furthermore, MiningTT Core will never create transactions paying less than
...
`miningTT.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
caching. A sample config for apache2 could look like:
    ...
    <Location /miningTTrpc>
        ...
        # optional bypass miningTTd rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< miningTTrpc:<password>
    </Location>
...
Restarting miningTTd will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in miningTT.conf
...
miningTT-cli: arguments privacy
------------------------------
...
For example:
    ...
    $ src/miningTT-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The MiningTT Core code base has
...
- `miningTT-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `miningTT-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the MiningTT scripting system that in combination with
...
MiningTT transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of MiningTT Core.
...
The two phases are distinct in the MiningTT-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming MiningTT wallet
  software more difficult and which seriously complicates the design of smart
  contracts for MiningTT.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each MiningTT block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of miningTTs they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of miningTTs being spent is exactly
...
- **Increased security for multisig:** MiningTT addresses (both P2PKH addresses
  ...
  believed to be provided by MiningTT's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of MiningTT's current ledger (called the UTXO set),
  ...
  note that MiningTT Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  MiningTT users to individually opt-in to almost any change in the MiningTT
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and MiningTT Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the MiningTT
client to MiningTT Core 0.13.2 to signal for segwit support. When a
...
Since MiningTT Core 0.10.0, nodes have defaulted to only relaying and
...
- `miningTT-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `miningTT-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting
